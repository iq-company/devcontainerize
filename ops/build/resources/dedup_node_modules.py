...
# Removing "name" from non-root package.json prevents phantom duplicates
# in CycloneDX / SPDX output.  Trivy needs *both* "name" + "version"
# to register a component – stripping "name" alone is sufficient.
#
# IMPORTANT: Do NOT strip "version" – some packages read their own version
# at runtime via require("../../package.json").version (e.g. @redis/client).
SBOM_FIELDS = {"name", "description", "author", "license",
...
# "type", "main", "module", "exports", "types", "typings", "browser",
# "engines", "version" (runtime reads!), "dependencies" (resolution)
...
# ---------------------------------------------------------------------------
# 7. Remove build-only packages (not needed at runtime)
# ---------------------------------------------------------------------------

# Frappe's Node.js runtime only uses a handful of packages (socket.io,
# @redis/client, cookie, superagent + transitive deps).  Everything else
# (esbuild, sass, postcss, vue compiler, glob, rimraf, …) is only needed
# during `bench build` to produce the browser bundles in sites/assets/.
#
# In the dev image, `yarn install` in init_site.sh / post_start.sh will
# restore anything missing.  In the release image, no builds ever happen.
#
# We remove packages that:
#   - have known CVEs AND are build-only, OR
#   - are large build tools with no runtime use
#
# NOTE: We only remove top-level installs. Nested copies inside runtime
# packages (e.g. glob inside socket.io) are left alone if they exist.

BUILD_ONLY_PACKAGES = {
    # Build tools / bundlers
    "glob",             # used by rimraf, stylus – build only; CVE via minimatch
    "rimraf",           # temp-file cleanup during build
    "tmp",              # temp dirs during build (depends on rimraf)
    "stylus",           # CSS preprocessor – output is in sites/assets/
    "sucrase",          # JS transpiler – build only
    # Large dev/build utilities
    "cypress",          # E2E testing framework (if present)
    "typescript",       # TS compiler – build only (if present)
    "loadjs",           # Script loader used by plyr – no upstream CVE fix (4.3.0 is latest)
    "jake",             # JS build tool used only for ejs's own build, not ours
    "filelist",         # Only used by jake
    # CSS/SCSS build chain (output is in sites/assets/)
    "sass",             # SCSS compiler – used by esbuild during bench build
    "immutable",        # Used by sass internally
    "svgo",             # SVG optimizer – used by postcss-svgo (cssnano) during build
    "postcss-svgo",     # PostCSS plugin for SVG optimization – build only
}


def remove_build_only(nm_dirs: list[Path]) -> tuple[int, int]:
    """Remove packages that are only needed during `bench build`.

    Only removes from top-level ``node_modules/<pkg>`` directories.
    ...
    for nm_dir in nm_dirs:
        for pkg_name in sorted(BUILD_ONLY_PACKAGES):
            pkg_path = nm_dir / pkg_name
            if not pkg_path.exists() or pkg_path.is_symlink():
                continue
            try:
                ...
                saved_bytes += size
                label = nm_dir.parent.name
                print(f"  {label}/{pkg_name} ({size / 1024:.0f} KB)")
            except OSError:
                pass
...
def main():
    ...
    # Phase 6: Remove build-only packages
    print("\n--- Phase 6: Remove build-only packages ---")
    build_rm, build_bytes = remove_build_only(nm_dirs)
    build_mb = build_bytes / (1024 * 1024)
    print(f"Removed {build_rm} build-only packages, saved ~{build_mb:.1f} MB")

    total_mb = (dedup_bytes + nested_bytes + dirs_bytes + files_bytes
                + prune_bytes + build_bytes) / (1024 * 1024)
