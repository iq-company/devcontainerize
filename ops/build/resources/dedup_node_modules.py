...
def _collect_all_packages(nm_dirs: list[Path]) -> dict[tuple[str, str], list[Path]]:
    ...
    canonical copy (preferring top-level over nested, then first-found order).
    """
    ...
    # Scan top-level first (so they appear first in lists → become canonical)
    for nm_dir in nm_dirs:
        for entry in sorted(nm_dir.iterdir()):
    ...
    # Then scan nested node_modules (these will be appended after top-level)
    for nm_dir in nm_dirs:
        for entry in sorted(nm_dir.iterdir()):
            if entry.is_symlink() or not entry.is_dir() or entry.name.startswith("."):
                continue
...
def dedup_nested(nm_dirs: list[Path]) -> tuple[int, int]:
    ...
    The first encountered copy (preferring top-level) becomes canonical;
    all others are replaced with relative symlinks.

    Returns (replaced_count, bytes_saved).
    """
    groups = _collect_all_packages(nm_dirs)
...
def main():
    ...
    nested_count, nested_bytes = dedup_nested(nm_dirs)
